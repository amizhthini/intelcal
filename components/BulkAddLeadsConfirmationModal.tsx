...
interface BulkAddLeadsConfirmationModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: () => void;
    stats: {
...
const BulkAddLeadsConfirmationModal: React.FC<BulkAddLeadsConfirmationModalProps> = ({ isOpen, onClose, onConfirm, stats }) => {
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md">
                ...
                <div className="p-6 space-y-4">
                    <p className="text-slate-700 dark:text-slate-300">
                        {stats.added > 0 ? `Successfully added ${stats.added} lead(s) to your list.` : 'No new leads were added.'}
                        {stats.skipped > 0 && ` Skipped ${stats.skipped} duplicate lead(s).`}
                    </p>
                    {stats.remaining > 0 && (
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-900/50 flex justify-end gap-2">
                    ...
                    <button onClick={onConfirm} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700">
                        Clear & Close
                    </button>
                </div>
            </div>
...
export default BulkAddLeadsConfirmationModal;